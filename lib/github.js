...
export async function getAllPosts() {
  try {
    ...
    // ✅ FIXED: Removed 'no-store', added revalidation
    const response = await fetch(url, {
      method: 'GET',
      headers: {
        'Authorization': `token ${TOKEN}`,
...
export async function getPost(slug) {
  try {
    if (!checkConfig()) throw new Error('GitHub configuration is incomplete');
...
export async function savePost(slug, content, metadata) {
  ...
  try {
    ...
    // Check if file exists (no-store is fine here for logic, but fetching relies on above)
    let sha = null;
    try {
      const url = `${GITHUB_API}/repos/${OWNER}/${REPO}/contents/${path}`;
      const existingFile = await fetch(url, {
        ...
        cache: 'no-store', // Keep no-store for admin saving actions
      });
...
export async function deletePost(slug) {
  ...
  try {
    ...
    if (!fileResponse.ok) throw new Error('Post not found');
    const fileData = await fileResponse.json();
